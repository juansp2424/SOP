  ...
  return (
    <Container>
      ...
      <MainContent>
        <FormContainer>
          <h2>Create SOP</h2>
          <h3>Step 4: Add reviewer</h3>

          <label>Name</label>
          <Input
            type="text"
            placeholder="Colleague X..."
            value={description}
          ...
          <Input
            type="text"
            placeholder="Please review step 1"
            value={requirements}
            onChange={(e) => setRequirements(e.target.value)}
          />

          <Button onClick={handleAddWorkflow}>Add additional reviewer</Button>
        </FormContainer>

        {/* Liste der hinzugefügten Workflows */}
        <WorkflowsContainer>
          <h3>Added reviewers</h3>
          {workflows.map((wf, index) => (
            <WorkflowItem key={index}>
              <p><strong>{wf.description}</strong> - Contact: {wf.responsible}</p>
              <p>Notification: {wf.requirements}</p>
            </WorkflowItem>
          ))}
        </WorkflowsContainer>
